...
sub show_bootp_dynamic {
    ...
    my ($button, $hidden_add_input_fields, $add_new_script) = &add_item_button(&text("add_", $text{"_networkid"}), $internalfield, $text{"p_desc_$internalfield"}, 700, 505, $formid, \@newfields );
    ...
    foreach my $item ( @{$dnsmconfig{$configfield}} ) {
        local %val = %{ $item->{"val"} };
        local @cols;
        ($edit_link[0], $hidden_edit_input_fields, $edit_script) = &edit_item_link($val, $internalfield, $text{"p_desc_$internalfield"}, $count, $formid, $w, $h, \@editfields);
