...
&ui_print_header($dnsmasq::text{"index_dns_records_settings"} . &icon_if_disabled($section),  $dnsmasq::text{"index_title"}, "", "intro", 1, 0, 0, &restart_button());
...
my @tabs = ( [ 'basic', $dnsmasq::text{'index_basic'} ],
             [ 'recs', $dnsmasq::text{"index_dns_records"} ],
        );
foreach my $v ( @vals ) {
...
&show_basic_fields( \%dnsmconfig, "dns_records", $page_fields, $apply_cgi . "?tab=basic", $dnsmasq::text{"index_basic"} );
print &ui_tabs_end_tab('tab', 'basic');

print &ui_tabs_start_tab('tab', 'recs');
&show_other_fields( \%dnsmconfig, "dns_records", $page_fields, $apply_cgi . "?tab=recs", $dnsmasq::text{"index_dns_records"} );
