#!/usr/bin/perl
#
#    DNSMasq Webmin Module - dhcp.cgi; DHCP vendor class config
...
print &ui_columns_start( [ $text{"class"},
                $text{"vendor"}, $text{"enabled"} ], 100 );
foreach my $range ( @{$dnsmconfig{"vendor-class"}} ) {
...
print "<br><a href=add.cgi?what=dhcp-vendorclass=new&where=dhcp.cgi>".
        $text{"add_"}." ".$text{"_vendor"}."</a><br><hr><br>";
