#!/usr/bin/perl
#
#    DNSMasq Webmin Module - # TODO dhcp_domain_edit.cgi;  edit DHCP domain
...
print "<br>".$text{"enabled"}.&ui_yesno_radio( "used",
                ($dnsmconfig{"domain"}[$in{idx}]->{"used"})?1:0 );
print "<br>".$text{"host_spec"};
print &ui_textbox( "host", $dnsmconfig{"domain"}[$in{idx}]->{"val"}, 80 );
...
# sub-routines
#
### END of dhcp_domain_edit_edit.cgi ###.
